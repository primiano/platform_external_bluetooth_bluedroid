...
void usb_close(void)
{
    ...
    USBDBG("usb_close \n");
ALOGD("xxxxxxx usb_close 1");
    usb_xfer_status |= RX_DEAD;

    if ((result=pthread_join(usb.read_thread, NULL)) < 0)
        ALOGE( "pthread_join() FAILED result:%d \n", result);
ALOGD("xxxxxxx usb_close 2");
    if (usb.handle) {
ALOGD("xxxxxxx usb_close 3");      
        libusb_release_interface(usb.handle, 1);
ALOGD("xxxxxxx usb_close 4");	
        libusb_release_interface(usb.handle, 0);
ALOGD("xxxxxxx usb_close 5");	
        libusb_close(usb.handle);
ALOGD("xxxxxxx usb_close 6");	
    }
ALOGD("xxxxxxx usb_close 7");    
    usb.handle = NULL;
    libusb_exit(NULL);
ALOGD("xxxxxxx usb_close 8");    
    if (bt_hc_cbacks)
    {
ALOGD("xxxxxxx usb_close 9");      
        while ((p_buf = utils_dequeue (&(usb.rx_eventq))) != NULL)
        {
            bt_hc_cbacks->dealloc(p_buf, (char *) ((RX_HDR *)p_buf+1));
        }
ALOGD("xxxxxxx usb_close 10");        
        while ((p_buf = utils_dequeue (&(usb.rx_isoq))) != NULL)
        {
            bt_hc_cbacks->dealloc(p_buf, (char *) ((RX_HDR *)p_buf+1));
        }
ALOGD("xxxxxxx usb_close 11");        
        while ((p_buf = utils_dequeue (&(usb.rx_bulkq))) != NULL)
        {
            bt_hc_cbacks->dealloc(p_buf, (char *) ((RX_HDR *)p_buf+1));
        }
ALOGD("xxxxxxx usb_close 12");        
    }
}
